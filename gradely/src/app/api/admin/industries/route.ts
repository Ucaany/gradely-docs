...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name?.trim()) return NextResponse.json<ApiResponse>({ data: null, error: 'Nama industri wajib diisi', success: false }, { status: 400 })

    const { data, error } = await supabase.from('industry_options').insert({ name: name.trim() }).select().single()
