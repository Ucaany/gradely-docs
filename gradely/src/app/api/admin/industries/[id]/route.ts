...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from('industry_options').update(updates).eq('id', params.id).select().single()
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabase.from('industry_options').delete().eq('id', params.id)
    if (error) return NextResponse.json<ApiResponse>({ data: null, error: error.message, success: false }, { status: 400 })
