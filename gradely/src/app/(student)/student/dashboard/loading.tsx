/**
 * loading.tsx - Student Dashboard Loading Screen
 *
 * Server Component async - await sleep agar loading screen
 * terlihat cukup lama sebelum konten muncul.
 */

import { MascotCycling } from '@/components/mascot/icons'

// Minimal delay agar loading screen tidak flash terlalu cepat
function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

export default async function StudentDashboardLoading() {
  await sleep(1800)

  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 min-h-[60vh]">
      <div className="animate-bounce">
        <MascotCycling className="w-24 h-24 text-black dark:text-neutral-100" />
      </div>
