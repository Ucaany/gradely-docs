/**
 * loading.tsx - Lecturer Dashboard Loading Screen
 */

import { MascotSittingReading } from '@/components/mascot/icons'

function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

export default async function LecturerDashboardLoading() {
  await sleep(1800)

  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 min-h-[60vh]">
      <div className="animate-bounce">
        <MascotSittingReading className="w-24 h-24 text-black dark:text-neutral-100" />
      </div>
